...
import com.example.portfolio.feature_myapp.presentation.view.adapter.CarouselAdapter
...
import com.example.portfolio.feature_myapp.presentation.viewmodel.MainStateEvent
import com.example.portfolio.feature_myapp.presentation.viewmodel.MyAppViewModel
...
class CarouselDisplayFragment : Fragment(R.layout.fragment_carousel_display) {
            ...
            viewModel.dataUIState.collect { dataState ->
                when(dataState){
                    ...
                    is DataState.Error -> {
                        ...
                        displayErrorMessage(dataState.message.toString())
                    }

                    is DataState.Success<List<MyAppLocal>> -> {
                        displayProgressBar(false)
                        dataState.data?.let { updateCarouselData(it) }
                    }
    ...
    private fun displayErrorMessage(exception:String){
        Toast.makeText(context,"Internet Error: $exception", Toast.LENGTH_SHORT).show()
    }
